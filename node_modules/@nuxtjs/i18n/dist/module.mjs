...
import { resolvePath, extendPages, addWebpackPlugin, extendWebpackConfig, addVitePlugin, extendViteConfig, defineNuxtModule, useLogger, isNuxt2, getNuxtVersion, isNuxt3, addPlugin, addTemplate, addImports } from '@nuxt/kit';
import { parse, resolve, relative, dirname, normalize, isAbsolute } from 'pathe';
...
function getProjectLayerI18n(configLayer) {
...
const mergeLayerLocales = (nuxt) => {
  ...
  const mergeSimpleLocales = () => {
    ...
    for (const layer of reversedLayers) {
      const i18n = getProjectLayerI18n(layer);
      debug$8("layer.config.i18n.locales", i18n?.locales);
...
const ResourceProxyPlugin = createUnplugin((options = {}, meta) => {
  ...
  const loader = await import(${toCode(withQuery(resolve$1(baseDir, query.target), { locale: query.locale }))}).then(m => m.default || m)
...
function generateAdditionalMessages(value, dev) {
  ...
  for (const [locale, messages] of Object.entries(value)) {
    ...
    for (const [, p] of Object.entries(messages)) {
      genCode += `() => Promise.resolve(${generateJSON(JSON.stringify(p), { type: "bare", env: dev ? "development" : "production", jit: true }).code}),`;
    }
    genCode += `],`;
  }
  genCode += "})";
